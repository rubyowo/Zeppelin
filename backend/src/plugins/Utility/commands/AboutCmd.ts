import type { APIEmbed, GuildChannel } from "discord.js";
import { shuffle } from "lodash-es";
import moment from "moment-timezone";
import { accessSync, readFileSync } from "node:fs";
import { rootDir } from "../../../paths.js";
import { getBotStartTime } from "../../../uptime.js";
import { noop, resolveMember, sorter } from "../../../utils.js";
...
export const AboutCmd = utilityCmd({
	...
	async run({ message: msg, pluginData }) {
		...
		const buildTimeMoment = buildTime
			? moment.utc(buildTime, "YYYY-MM-DDTHH:mm:ss[Z]")
			: null;
		...
		const basicInfoRows = [
			...
			[
				"Last config reload",
				`<t:${Math.floor(pluginData.state.lastReload / 1000)}:R>`,
			],
			[
				"Last bot update",
				buildTimeMoment
					? `<t:${Math.floor(buildTimeMoment.unix())}:f>`
					: "Unknown",
			],
